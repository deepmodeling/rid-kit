information = """
#########################################################################################

                              Reinforced Dynamics Toolkit

Rid-kit is a python package for enhanced sampling via the RiD (Reinforced Dynamics) method.

Repos of RiD-kit are maintained by DeepModeling Community on Github: 
    https://github.com/deepmodeling/rid-kit

When you use rid-kit, you agree with GNU Lesser General Public License v3.0.

Please Cite:
    
    Zhang, L., Wang, H., E, W.. Reinforced dynamics for enhanced sampling in large atomic and molecular systems[J]. The Journal of chemical physics, 2018, 148(12): 124113.
    
    Wang, D., Wang, Y., Chang, J. et al. Efficient sampling of high-dimensional free energy landscapes using adaptive reinforced dynamics. Nat Comput Sci 2, 20–29 (2022). https://doi.org/10.1038/s43588-021-00173-1

Let us know if you have any question. Feel free to open issues on Github.

Welcome to reach out to :
    yanze039@mit.edu
    wangdd@dp.tech
    
"""