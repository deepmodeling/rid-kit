...
class Model(object):
    ...
    def train(self, reader):
        ...
        self.inputs_train = tf.placeholder(tf_precision, [None, self.n_input + self.cv_dim], name='inputs')
    ...
    def build_force (self, 
        ...
        dists = tf.slice(cvs, [0,self.cv_dih_dim], [-1,self.cv_dist_dim], name='dists')
        ...
        forces = - tf.reshape(tf.stack(tf.gradients(energy, cvs)), [-1, self.cv_dim], name='o_forces')
    ...
    def _batch_norm(self, x, name, reuse):
        ...
        with tf.variable_scope(name+'moving', reuse=False):
            moving_mean = tf.get_variable('moving_mean', params_shape, tf_precision,
                                          initializer=tf.constant_initializer(0.0, tf_precision),
                                          trainable=False)
            moving_variance = tf.get_variable('moving_variance', params_shape, tf_precision,
                                              initializer=tf.constant_initializer(1.0, tf_precision),
                                              trainable=False)
