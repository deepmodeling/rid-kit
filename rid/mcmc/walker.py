...
class Walker(object):
    def __init__(self, fd, nw, sess, type):
        self.type = type
        self._full_dim = fd
        self._num_walker = nw
        self._move_scale = 0.1
        self._sess = sess
        self._shape = (self._num_walker, self._full_dim)
        if self.type == "dih":
            self._cv_range = 2*np.pi
        elif self.type == "dis":
            self._cv_range = 10
        else:
    ...
    def sample(self, compute_ef, inter_step=1):
        ...
        for _ in range(inter_step):
            if self.type == "dih":
                ...
                position_new = self._position + np.random.normal(scale=self._move_scale, size=self._shape)
            else:
                raise ValueError("Undefined cv type, only support 'dih' and 'dis' type")
...
def my_hist2d_path(pp, xx, yy, delta, lm, xlist):
    my_hist = np.zeros((1, len(xx), len(yy)))
    cv = pp[:,:4]
    slist = s(cv, lm, xlist)
    zlist = z(cv, lm, xlist)
    for ii in range(cv.shape[0]):
        if 0 < slist[ii] < 10 and -8 < zlist[ii] < 2:
            my_hist[0, np.int((slist[ii])//delta), np.int((zlist[ii]+8)//delta)] += 1
    my_hist /= (pp.shape[0] * delta * delta)
    return my_hist