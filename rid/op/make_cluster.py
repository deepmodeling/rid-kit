...
from rid.constants import culster_selection_data_name, culster_selection_index_name
import numpy as np
...
class PrepSelector(OP):
    ...
    def execute(
        ...
        threshold = cv_cluster.make_threshold(op_in["numb_cluster_upper"], op_in["numb_cluster_lower"])
        cls_sel = cv_cluster.get_cluster_selection()
        save_txt(culster_selection_index_name, cls_sel)
        selected_data = data[cls_sel]
        save_txt(culster_selection_data_name, selected_data)
        op_out = OPIO({
